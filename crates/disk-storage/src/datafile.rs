...
use alloc::boxed::Box;
use spacetimedb_runtime_io::{AlignedBytes, ReadWriteResult, SpacetimeIO};
...
impl<'a, IO: SpacetimeIO> Datafile<'a, IO> {
    pub async fn open(io: &'a IO, path: Box<str>) -> Result<Self, IO::Error> {
        let fd = io.open_file(path).await?;

        Ok(Self { io, fd })
    }

    pub async fn create(io: &'a IO, path: Box<str>) -> Result<Self, IO::Error> {
    ...
    pub(crate) fn write_all_at<B: AlignedBytes + Send + 'static>(
        ...
        buf: Box<B>,
        offset: u64,
    ) -> IO::Completion<ReadWriteResult<B, IO::Error>> {
