...
use spacetimedb_runtime_io::{AlignedBytes, ErrorWith, SpacetimeIO};
...
impl ChainedBlock {
    ...
    pub async fn load_into<IO: SpacetimeIO>(
        ...
        ptr: &BlockRef,
        buf: Box<Self>,
    ) -> Result<Box<Self>, ErrorWith<ReadError<IO>, Box<Self>>> {
        let block = df
    ...
    pub fn load_chain<IO: SpacetimeIO>(
        df: &Datafile<'_, IO>,
        mut mem: impl FnMut() -> Box<ChainedBlock>,
        head: BlockRef,
    ) -> impl Stream<Item = Result<Box<Self>, ErrorWith<ReadError<IO>, Box<Self>>>> {
        stream! {
