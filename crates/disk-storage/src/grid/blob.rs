...
use spacetimedb_runtime_io::{ErrorWith, SpacetimeIO};
...
pub async fn load_into<IO: SpacetimeIO>(
    df: &Datafile<'_, IO>,
    ptr: &BlobRef,
    buf: Box<ChainedBlock>,
    dst: &mut [u8],
) -> Result<Box<ChainedBlock>, ErrorWith<ReadError<IO>, Box<ChainedBlock>>> {
...
impl<'a, IO: SpacetimeIO> PendingBlob<'a, IO> {
    ...
    fn submit_write(
        ...
        mut memory: Box<ChainedBlock>,
    ) -> Result<InFlightBlock<Box<ChainedBlock>, IO>, Box<ChainedBlock>> {
    ...
    fn complete_write(
        &mut self,
        result: Result<Box<ChainedBlock>, ErrorWith<IO::Error, Box<ChainedBlock>>>,
    ) -> Box<ChainedBlock> {
        self.completed += 1;
        match result {
            Ok(block) => block,
    ...
    IO: SpacetimeIO,
{
    fn new(
        ...
        block_memory: &'a mut ArrayVec<Box<ChainedBlock>, MAX_CONCURRENCY>,
...
type Completions<const MAX_CONCURRENCY: usize, IO> =
    [Option<(usize, InFlightBlock<Box<ChainedBlock>, IO>)>; MAX_CONCURRENCY];
...
struct StoreBlobsInner<'a, 'io, 'free_set, 'jobs, const MAX_CONCURRENCY: usize, const JOB_COUNT: usize, IO: SpacetimeIO>
{
    ...
    block_memory: &'a mut ArrayVec<Box<ChainedBlock>, MAX_CONCURRENCY>,
    jobs: [PendingBlob<'jobs, IO>; JOB_COUNT],
    completions: Completions<MAX_CONCURRENCY, IO>,
...
pub async fn store_blobs<const MAX_CONCURRENCY: usize, const JOB_COUNT: usize, IO>(
    ...
    reserved_addresses: Reservation,
    block_memory: &mut ArrayVec<Box<ChainedBlock>, MAX_CONCURRENCY>,
